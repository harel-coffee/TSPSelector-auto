                    ...
                    with open(file, 'r') as fp:
                        ...
                        for line in lines:
                            ...
                            if status in ['TIMEOUT', 'Unsuccessful']:
                                runtime = tmax
                            data[n, nn, nnn] = (
                                tu[0], insts[n], runtime, 0, status)
                        if not flag:
                            print(
                                "Not found result for SMAC/PARAMILS in %s" % (file))
                            needtest.append((n, nn, nnn))
                            continue
                else:
...
def summary():
    out_dir = 'data/TSP/additional/runs/'
    result_files = glob('%s*_algorithm_runs*' % out_dir)
    f = open('data/TSP/additional/algorithm_runs.csv', 'w+')
...
if __name__ == '__main__':
    ...
    # instances = []
    # instances.extend(glob('data/TSP/additional/RUE/*'))
    # instances.extend(glob('data/TSP/additional/cl/*'))
    # instances.extend(glob('data/TSP/additional/explosion/*'))
    # instances.extend(glob('data/TSP/additional/implosion/*'))
    # instances.extend(glob('data/TSP/additional/cluster/*'))
    # instances.extend(glob('data/TSP/additional/compression/*'))
    # instances.extend(glob('data/TSP/additional/rotation/*'))
    # instances.extend(glob('data/TSP/additional/expansion/*'))
    # instances.extend(glob('data/TSP/additional/linearprojection/*'))
    # instances.extend(glob('data/TSP/additional/grid/*'))
    for option in options:
        output_dir = 'data/TSP/additional/runs/%s/' % option
        ...
        algos.append(("LKH", "python -u solver/LKH/wrapper.py --mem-limit 2048"
                             " --solutionity data/TSP/additional/optimum.json "))
        ...
        while need_test_ones:
            ...
            while processSet:
                time.sleep(5)
                print('Still %d sub process not exits' % len(processSet))
                finished = [pid for pid in processSet if pid.poll()
                            is not None]
                processSet -= set(finished)
